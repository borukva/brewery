    ...
    record ExecuteCommand(String command, WrappedExpression apply) implements ConsumptionEffect {
        ...
        public void apply(LivingEntity user, double age, double quality) {
            ...
            if (value >= 0) {
                ...
                var source = user.getCommandSource((ServerWorld) user.getEntityWorld()).withLevel(4).withOutput(CommandOutput.DUMMY);

                var parseResults = server.getCommandManager().getDispatcher().parse(this.command, source);
                server.getCommandManager().execute(parseResults, this.command);
            }
        }
